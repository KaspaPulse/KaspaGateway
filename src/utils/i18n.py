...
def _load_translations() -> None:
    ...
    logger.info(f"Translations loaded for language: '{_current_lang_code}' from {TRANSLATIONS_DIR}")
...
def switch_language(lang_code: str) -> bool:
    ...
    if lang_code and lang_code in LANG_MAP:
        if lang_code != _current_lang_code:
            logger.info(f"Switching language from '{_current_lang_code}' to '{lang_code}'")
...
def get_available_languages() -> List[Dict[str, str]]:
    ...
    langs: List[Dict[str, str]] = [{'code': code, 'name': name} for code, name in LANG_MAP.items()]
    
    # Sort by display name
    langs.sort(key=lambda x: x['name'])
    
    # Ensure 'en' is always first in the list
    en_lang: Optional[Dict[str, str]] = next((lang for lang in langs if lang['code'] == 'en'), None)
    if en_lang:
