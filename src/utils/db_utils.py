...
def retry_on_schema_error(
    schema_init_func: Callable[[DuckDBPyConnection], None],
        ...
        def wrapper(self: "DatabaseManager", *args: Any, **kwargs: Any) -> T:
            ...
            try:
                ...
                if (
                    ...
                    logger.warning(
                        f"Schema error in {func.__name__} for {db_path}: {e}. Retrying after re-initializing schema..."
                    )
                    try:
                        ...
                        logger.error(
                            f"Failed to re-initialize schema or retry function {func.__name__}: {e2}",
                            exc_info=True,
                        )
                ...
                logger.error(
                    f"Unexpected error in decorated function {func.__name__}: {e}",
                    exc_info=True,
                )
                raise e
