...
try:
    ...
    logger.info(
        f"PERF: reportlab imported successfully in {t_end_reportlab - t_start_reportlab:.4f} seconds."
    )

except ImportError:
    REPORTLAB_AVAILABLE = False
    logger.warning(
        "PDF export functionality is disabled. Required libraries are missing."
    )
...
class ReportPDFTemplate(BaseDocTemplate):
    ...
    def _footer(self, canvas: "Canvas", doc: Any) -> None:
        ...
        export_time_text: str = (
            f"{translate('Exported On')}: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        )
