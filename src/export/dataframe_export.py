...
from src.config.config import CONFIG, APP_NAME, get_active_api_config, get_assets_path
from src.export.pdf_utils import ReportPDFTemplate, create_paragraph, REPORTLAB_AVAILABLE
...
def _create_report_header_text(kaspa_address: str, address_name: str, currency: str) -> str:
    """
    Generates the comment header for CSV exports.
    """
    timestamp_str = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    header = f'# {APP_NAME} {translate("Version")} {CONFIG.get("version", "N/A")}\n'
    header += f'# {translate("Transaction Report for Kaspa Address")}: {kaspa_address}\n'
    if address_name:
...
def _prepare_dataframe_for_export(
    ...
    core_cols = ['timestamp', 'txid', 'direction', 'from_address', 'to_address', 'amount', 'block_height', 'type']
        ...
        if addr_col in export_df.columns:
            if sanitize_for_csv:
                export_df[addr_col] = export_df[addr_col].apply(
                    lambda x: ', '.join(sanitize_csv_cell(known_names_map.get(addr, addr)) for addr in str(x).split(', '))
                )
            else:
                export_df[addr_col] = export_df[addr_col].apply(
                    lambda x: ', '.join(html.escape(known_names_map.get(addr, addr)) for addr in str(x).split(', '))
                )
...
def export_df_to_csv(
    ...
    **kwargs: Any
    ...
    if not file_path.lower().endswith('.csv'): 
        file_path += '.csv'
    try:
        export_df, value_col_name, rename_map = _prepare_dataframe_for_export(df, currency, known_names_map, sanitize_for_csv=True)
        
        if value_col_name in export_df.columns:
            export_df[value_col_name] = export_df[value_col_name].apply(lambda x: f"{x:,.2f} {currency.upper()}" if pd.notnull(x) else "N/A")
        
        if (dir_col := rename_map.get('direction')) in export_df.columns:
            export_df[dir_col] = export_df[dir_col].apply(lambda x: sanitize_csv_cell(translate(str(x).capitalize())))
        if (type_col := rename_map.get('type')) in export_df.columns:
            export_df[type_col] = export_df[type_col].apply(lambda x: sanitize_csv_cell(translate(str(x).capitalize())))
...
def export_df_to_html(
    ...
    **kwargs: Any
    ...
    if not file_path.lower().endswith('.html'): 
        file_path += '.html'
    try:
        export_df, value_col_name, rename_map = _prepare_dataframe_for_export(df, currency, known_names_map)
        api_config = get_active_api_config()
        tx_url_base: str = api_config.get('explorer', {}).get('transaction', '')

        if value_col_name in export_df.columns:
            export_df[value_col_name] = export_df[value_col_name].apply(lambda x: f"{x:,.2f} {html.escape(currency.upper())}" if pd.notnull(x) else "N/A")

        tx_id_col = rename_map.get('txid')
        if tx_id_col in export_df.columns and tx_url_base and 'txid' in df.columns:
            export_df[tx_id_col] = df['txid'].apply(lambda x: f'<a href="{tx_url_base.format(txid=x)}" target="_blank">{html.escape(str(x))}</a>' if x else "")
        
        if (dir_col := rename_map.get('direction')) in export_df.columns:
            export_df[dir_col] = export_df[dir_col].apply(lambda x: html.escape(translate(str(x).capitalize())))
        if (type_col := rename_map.get('type')) in export_df.columns:
            export_df[type_col] = export_df[type_col].apply(lambda x: html.escape(translate(str(x).capitalize())))

        is_rtl: bool = CONFIG.get("language", "en") == "ar"
        body_tag: str = f'<body class="{"rtl" if is_rtl else ""}">'

        html_content: str = export_df.to_html(index=True, index_label=translate('No.'), escape=False, classes='kaspa-table', justify='left')
        ...
        """
        footer: str = f'<footer><p>{APP_NAME} {translate("Version")} {CONFIG.get("version", "N/A")}</p></footer>'
        full_html: str = f'<!DOCTYPE html><html><head><meta charset="UTF-8"><title>{html.escape(report_title)}</title>{STYLE}</head>{body_tag}{header}{html_content}{footer}</body></html>'
...
def export_df_to_pdf(
    ...
    **kwargs: Any
    ...
    try:
        ...
        story: List[Any] = [create_paragraph(translate("Kaspa Transaction Report"), styles['h1'], is_rtl)]
        story.append(create_paragraph(f"{translate('Kaspa Address')}: {kaspa_address}", styles['Normal'], is_rtl))
        if address_name:
            story.append(create_paragraph(f"({translate('Address Name')}: {address_name})", styles['Normal'], is_rtl))
        ...
        if (amount_col := rename_map.get('amount')) in export_df.columns:
            export_df[amount_col] = export_df[amount_col].apply(lambda x: f"{x:,.8f}" if pd.notnull(x) else "N/A")
        
        if (dir_col := rename_map.get('direction')) in export_df.columns:
            export_df[dir_col] = export_df[dir_col].apply(lambda x: translate(str(x).capitalize()))
        if (type_col := rename_map.get('type')) in export_df.columns:
            export_df[type_col] = export_df[type_col].apply(lambda x: translate(str(x).capitalize()))

        data_list: List[List[Any]] = [export_df.columns.tolist()] + cast(List[List[Any]], export_df.values.tolist())
        ...
        col_widths: List[float] = [doc_width * relative_widths.get(str(col), 0.1) for col in export_df.columns]
        ...
        if not is_rtl:
            col_map: Dict[str, int] = {name: i for i, name in enumerate(export_df.columns)}
            if (dir_col := rename_map.get('direction')) in col_map: 
                style_commands.append(('ALIGN', (col_map[dir_col], 1), (col_map[dir_col], -1), 'CENTER'))
            if (type_col := rename_map.get('type')) in col_map: 
                style_commands.append(('ALIGN', (col_map[type_col], 1), (col_map[type_col], -1), 'CENTER'))
            if (amount_col := rename_map.get('amount')) in col_map: 
                style_commands.append(('ALIGN', (col_map[amount_col], 1), (col_map[amount_col], -1), 'RIGHT'))
            if value_col_name in col_map: 
                style_commands.append(('ALIGN', (col_map[value_col_name], 1), (col_map[value_col_name], -1), 'RIGHT'))
