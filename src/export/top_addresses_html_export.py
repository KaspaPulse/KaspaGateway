...
from src.config.config import CONFIG, APP_NAME, APP_VERSION, get_active_api_config
...
def export_top_addresses_to_html(df: pd.DataFrame, file_path: str, currency: str) -> Tuple[bool, str, str]:
    ...
    try:
        ...
        export_df['Known Name'] = export_df['Known Name'].apply(lambda x: html.escape(str(x)))
        api_config = get_active_api_config()
        addr_url_base = api_config['explorer']['address']
        export_df['Address'] = export_df['Address'].apply(lambda x: f'<a href="{addr_url_base.format(kaspaAddress=x)}" target="_blank">{x}</a>')
        currency_upper = currency.upper()
        value_col_name = translate(f"Value ({currency_upper})")
        if 'Value' in export_df.columns:
            export_df['Value'] = export_df['Value'].apply(lambda x: f"{x:,.2f} {currency_upper}")
        rename_map = {
            'Rank': translate('Rank'),
        ...
        final_cols = [
            ...
            translate('Balance (KAS)'), value_col_name
        ]
        html_table = export_df[final_cols].to_html(index=False, border=0, classes='kaspa-table', escape=False, float_format='%.2f')
