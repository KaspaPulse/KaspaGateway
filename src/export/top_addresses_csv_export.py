...
def export_top_addresses_to_csv(
    ...
    try:
        ...
        export_df["Address URL"] = export_df["Address"].apply(
            lambda x: addr_url_base.format(kaspaAddress=x)
        )
        ...
        if "Value" in export_df.columns:
            export_df["ValueFormatted"] = export_df["Value"].apply(
