...
class TopAddressesTab(ttk.Frame):
    ...
    def _build_ui(self) -> None:
        ...
        self.last_updated_label = ttk.Label(
            header_frame, text=f"{translate('Last Updated')}: N/A"
        )
        ...
        self.save_hint_tooltip = ToolTip(
            hint_label,
            text=translate("Right-click an address to add it to 'My Addresses'."),
        )
        ...
        self.refresh_button = ttk.Button(
            button_frame, text=translate("Refresh List"), command=self.refresh_data
        )
        ...
        self.search_button = ttk.Button(
            filter_frame, text=translate("Filter"), command=self._apply_filters_and_sort
        )
        self.search_button.grid(row=0, column=1, padx=(0, 5))

        self.reset_button = ttk.Button(
            filter_frame, text=translate("Reset Filter"), command=self._reset_filters
        )
    ...
    def refresh_data(self) -> None:
        ...
        self._thread = threading.Thread(
            target=self._fetch_worker, daemon=True, name="TopAddressesThread"
        )
        self._thread.start()
    ...
    def _apply_filters_and_sort(self) -> None:
        ...
        if search_term and not self.placeholder_active and not df.empty:
            df = df[
                df.apply(
                    lambda row: search_term in str(row["Address"]).lower()
                    or search_term in str(row["Known Name"]).lower()
                    or search_term == str(row["Rank"]),
                    axis=1,
                )
            ]
        ...
        if not df.empty:
            ...
            self.last_updated_label.config(
                text=f"{translate('Last Updated')}: {time.strftime('%Y-%m-%d %H:%M:%S')}"
            )
        else:
            ...
            try:
                ...
                if balance_str:
                    ...
                    self.tree.set(
                        item_id,
                        column="value",
                        value=f"{new_value:,.2f} {new_currency.upper()}",
                    )
    ...
    def re_translate(self) -> None:
        ...
        self.save_hint_tooltip.text = translate(
            "Right-click an address to add it to 'My Addresses'."
        )
        self._configure_tree_headings()
        ...
        self.context_menu.add_command(
            ...
            command=self._add_selected_to_my_addresses,
        )
        self.context_menu.add_command(
        ...
        if self.full_df.empty:
            self.show_placeholder(
                translate("Press 'Refresh List' to load top addresses.")
            )
    ...
    def show_placeholder(self, message: str) -> None:
        ...
        self._clear_tree()
        self.tree.insert(
            "", "end", values=("", "", message, "", ""), tags=("placeholder",)
        )
