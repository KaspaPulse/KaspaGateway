...
class ManualCalendarPopup(ttk.Toplevel):
    ...
    def __init__(
        ...
        # Month names for display (index 0 is empty)
        self.month_names = [
...
class _AnalysisFilterControls(ttk.Labelframe):
    ...
    def _build_ui(self) -> None:
        ...
        self.search_entry.grid(
            row=0, column=2, columnspan=3, padx=(10, 5), pady=5, sticky="ew"
        )
...
class NormalAnalysisTab(ttk.Frame):
    ...
    def _build_normal_analysis_tab(self, parent_frame: ttk.Frame) -> None:
        ...
        self.normal_analysis_prog_bar = ttk.Progressbar(
            controls_frame, mode="indeterminate", bootstyle="info-striped", length=150
        )
        ...
        self.normal_summary_labelframe.grid(
        ...
        self.show_normal_placeholder(translate("Load an address to see transactions."))
    ...
    def _show_normal_context_menu(self, event: tk.Event) -> None:
        ...
        item_id = self.normal_tree.identify_row(event.y)
        if not item_id:
            return
    ...
    def _normal_add_selected_to_my_addresses(self) -> None:
        ...
        sel = self.normal_tree.selection()
        if not sel:
            return
    ...
    def _normal_copy_selected_address(self) -> None:
        ...
        if not sel:
            return
        values = self.normal_tree.item(sel[0], "values")
    ...
    def _build_normal_tree_view(self, parent: ttk.Frame) -> None:
        ...
        vsb = ttk.Scrollbar(
            self.normal_tree_labelframe, orient=VERTICAL, command=self.normal_tree.yview
        )
        hsb = ttk.Scrollbar(
            self.normal_tree_labelframe,
            orient=HORIZONTAL,
    ...
    def _update_normal_analysis_controls(self) -> None:
        ...
        self.cancel_analysis_button.config(state=NORMAL if is_running else DISABLED)
    ...
    def run_analysis(self) -> None:
        ...
        filters = self.normal_filter_controls.get_filters()
        threading.Thread(
    ...
    def _normal_analysis_worker(self, filters: Dict[str, Any]) -> None:
        ...
        """
        try:
            if not self.main_window.tx_db:
                return
                
            df_list = self.main_window.tx_db.filter_transactions(
            ...
            df["flow"] = df.apply(
                lambda row: (
                    row["amount"] if row["direction"] == "incoming" else -row["amount"]
                ),
                axis=1,
            )
            ...
            for row_dict in df.to_dict("records"):
                if row_dict.get("type") == "coinbase":
                    ...
                    for addr in addresses:
                        if (
                            addr
                            and addr != self.main_address
                            and addr != "N/A (Coinbase)"
                        ):
            ...
            summary_snapshot = {
                ...
                "Avg Inflow (KAS)": (
                    f"{summary['avg_inflow']:,.2f}"
                    if pd.notna(summary["avg_inflow"])
                    else "0.00"
                ),
                "Avg Outflow (KAS)": (
                    f"{summary['avg_outflow']:,.2f}"
                    if pd.notna(summary["avg_outflow"])
                    else "0.00"
                ),
                "Total Transactions": f"{summary['total_transactions']:,}",
                "Largest Inflow (KAS)": (
                    f"{summary['max_inflow']:,.2f}"
                    if pd.notna(summary["max_inflow"])
                    else "0.00"
                ),
                "Largest Outflow (KAS)": (
                    f"{summary['max_outflow']:,.2f}"
                    if pd.notna(summary["max_outflow"])
                    else "0.00"
                ),
                "Unique Counterparties": f"{len(counterparties):,}",
                "Duration (Days)": f"{duration:,}",
                "First Transaction": (
                    first_tx_date.strftime("%Y-%m-%d")
                    if pd.notna(first_tx_date)
                    else "N/A"
                ),
                "Last Transaction": (
                    last_tx_date.strftime("%Y-%m-%d")
    ...
    def update_normal_ui(
        ...
        self._update_normal_treeview_progressively(results.get("counterparties", {}))
    ...
    def _update_normal_treeview_progressively(
        ...
        for address, tx_list in counterparties.items():
            ...
            if self.normal_tree.exists(address):
                self.normal_tree.item(address, text=display_name, values=values_tuple)
            else:
                parent_id = self.normal_tree.insert(
                    "",
                    "end",
                    iid=address,
                    text=display_name,
    ...
    def _on_normal_tree_open(self, event: Any) -> None:
        ...
        item_id = self.normal_tree.focus()
        if not item_id:
            return

        child_nodes = self.normal_tree.get_children(item_id)
        if not child_nodes:
            return

        first_child_id = child_nodes[0]
        ...
        self.after(
            10, self._load_normal_analysis_batch, item_id, tx_iterator, price, currency
        )
    ...
    def _load_normal_analysis_batch(
        ...
        if not self.normal_tree.exists(item_id):
            logger.warning(
                f"Batch load cancelled: Parent item {item_id} no longer exists."
            )
            return
            ...
            try:
                ...
                self.normal_tree.insert(
                    ...
                    values=(
                        ...
                        (
                            f"{value:,.2f} {currency.upper()}"
                            if tx.get(f"value_{currency.lower()}") is not None
                            else "N/A"
                        ),
        ...
        self.after(
            20, self._load_normal_analysis_batch, item_id, tx_iterator, price, currency
        )
    ...
    def _on_normal_tree_close(self, event: Any) -> None:
        ...
        first_child_id = self.normal_tree.get_children(item_id)[0]
        if first_child_id == f"child_{item_id}":
            return
    ...
    def show_normal_placeholder(self, message: str, clear_address: bool = True) -> None:
    ...
    def _configure_normal_tree_headings(self) -> None:
        ...
        currency = self.main_window.currency_var.get().upper()
        ...
        self.normal_tree.heading(
            "c3", text=f"{translate('Net Flow (KAS)')} / {translate('Amount (KAS)')}"
        )
    ...
    def _re_translate_normal_context_menu(self) -> None:
        ...
        self.normal_context_menu.add_command(
            label=translate("Copy Address"), command=self._normal_copy_selected_address
        )
    ...
    def export_normal_analysis_data(self, export_format: str) -> None:
        ...
        export_args = {
            ...
            "analysis_data": {"summary": self.analysis_results.get("summary")},
        }
