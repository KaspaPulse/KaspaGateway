...
from src.utils.i18n import translate, get_available_languages
...
class SettingsApiPerfTab(ttk.Frame):
    ...
    def __init__(self, parent: ttk.Frame, main_window: 'MainWindow', settings_tab: 'SettingsTab') -> None:
        super().__init__(parent)
        self.main_window: 'MainWindow' = main_window
        self.config_manager = main_window.config_manager

        # Get state variables from the parent SettingsTab
        ...
        self.log_level_var: ttk.StringVar = settings_tab.log_level_var
    ...
    def _build_general_settings(self, parent: ttk.Frame) -> None:
        ...
        display_options_frame = ttk.Labelframe(parent, text=translate("Display Settings"), padding=10)
        display_options_frame.grid(row=0, column=0, sticky='nsew', pady=(10, 15), padx=10)
        display_options_frame.grid_columnconfigure((0, 1, 2), weight=1, uniform="group1")
        self.labelframes['Display Settings'] = display_options_frame

        lang_items: Dict[str, str] = {lang['code']: f"{translate(lang['name'])} ({lang['code']})" for lang in get_available_languages()}
        curr_items: Dict[str, str] = {code: f"{translate(CURRENCY_TRANSLATION_KEYS.get(code, code.upper()))} ({code.upper()})" for code in SUPPORTED_CURRENCIES}
        tab_items: Dict[str, str] = {tab_name: translate(tab_name) for tab_name in SUPPORTED_TABS}

        self._setup_selectable_list(display_options_frame, 0, "Displayed Languages", lang_items, "lang_vars", None, num_cols=2)
        self._setup_selectable_list(display_options_frame, 1, "Displayed Currencies", curr_items, "currency_vars", None, num_cols=3)
        self._setup_selectable_list(display_options_frame, 2, "Displayed Tabs", tab_items, "tab_vars", "tab_cbs", num_cols=2)
        ...
        advanced_frame = ttk.Labelframe(bottom_container, text=translate("Advanced"), padding=10)
        advanced_frame.grid(row=0, column=0, sticky="nsew", padx=(0, 5))
        advanced_frame.grid_columnconfigure(1, weight=1)
        self.labelframes['Advanced'] = advanced_frame

        log_level_label = ttk.Label(advanced_frame, text=f"{translate('Logging Level')}:")
        log_level_label.grid(row=0, column=0, sticky='w', padx=5, pady=5)

        log_level_combo = ttk.Combobox(advanced_frame, textvariable=self.log_level_var, values=["DEBUG", "INFO", "WARNING", "ERROR"], state="readonly", width=15)
        log_level_combo.grid(row=0, column=1, sticky='w', padx=5, pady=5)
        self.entries[("logging_level",)] = self.log_level_var

        check_updates_cb = ttk.Checkbutton(advanced_frame, text=translate("Check for updates on startup"), variable=self.check_updates_var)
        ...
        self.autostart_cb = ttk.Checkbutton(advanced_frame, text=translate("Start with Windows"), variable=self.autostart_var)
        self.autostart_cb.grid(row=2, column=0, columnspan=2, sticky='w', padx=5, pady=5)
        ...
        path_frame = ttk.Labelframe(bottom_container, text=translate("File Paths"), padding=10)
    ...
    def _build_performance_settings(self, parent: ttk.Frame) -> None:
        ...
        self.auto_refresh_cb = ttk.Checkbutton(auto_lf, text=translate("Enable Auto-Refresh"), variable=self.auto_refresh_var)
    ...
    def _is_private_ip(self, url: str) -> bool:
        ...
        try:
            ...
            # If domain resolution fails or it's not an IP, treat as potentially private/local for safety
            return True
        except Exception:
            return True  # Default to safe (private)
    ...
    def _on_base_url_change(self, *args: Any) -> None:
        ...
        current_valid_base: str = _get_nested_value(CONFIG, ("api", "profiles", profile_name, "base_url"), "")
        ...
        config_path: Tuple[str, ...] = ("api", "profiles", profile_name, "base_url")
    ...
    def _create_path_setting_row(
        ...
        browse_btn = ttk.Button(entry_frame, text="📂", command=lambda w=widget: self._browse_directory(w), width=3)
        browse_btn.grid(row=0, column=1, padx=(5, 0))

        clear_btn = ttk.Button(entry_frame, text="🗑️", command=lambda w=widget, key=folder_key: self._clear_folder_contents(w, key), width=3)
    ...
    def _clear_folder_contents(self, path_entry_widget: ttk.Entry, folder_key: str) -> None:
        ...
        try:
            user_data_root_db: str = self.config_manager.get_config().get('paths', {}).get('database', '')
            user_data_root: str = os.path.abspath(os.path.dirname(user_data_root_db)) if user_data_root_db else ''
            
            safe_path: str = user_data_root
            target_path: str = os.path.abspath(folder_path)

            if not safe_path or not target_path or not os.path.commonpath([safe_path]) == os.path.commonpath([safe_path, target_path]):
                messagebox.showerror(translate("Error"), translate("Deletion outside user data directory is not allowed."))
                return
        except Exception as e:
            logger.error(f"Security check failed during folder clearing: {e}")
            messagebox.showerror(translate("Error"), translate("Security check failed."))
            return

        folder_name: str = os.path.basename(os.path.normpath(folder_path))

        if folder_key == 'log':
            if not messagebox.askyesno(translate("Confirm Deletion"), translate("Clear Logs Warning")):
                return
        ...
        if not messagebox.askyesno(translate("Confirm Deletion"), f"{translate('Clear Folder Warning').format(folder_name)}"):
            return
    ...
    def _clear_logs_worker(self, folder_path: str) -> None:
        ...
        self.after(0, self.main_window.status.update_status, f"{translate('Clearing folder:')} {os.path.basename(folder_path)}...")
        try:
            ...
            for item in os.listdir(folder_path):
                ...
                if not os.path.islink(item_path) and os.path.isfile(item_path) and item.lower().endswith('.log'):
                    os.unlink(item_path)
            ...
            self.after(0, lambda: ToastNotification(title=translate("Error"), message=translate("Failed to delete files. Check logs."), bootstyle=DANGER, duration=3000).show_toast())
        finally:
        ...
        try:
            for item in os.listdir(folder_path):
                ...
                if os.path.isfile(item_path) and not os.path.islink(item_path):
                    os.unlink(item_path)
                elif False and os.path.isdir(item_path) and not os.path.islink(item_path):
                    # Safety: currently only files are deleted, not subfolders
    ...
    def _populate_api_tree(self) -> None:
        ...
        base_url_parent: str = self.api_tree.insert("", "end", text="Base URL", open=True, iid="base_url_cat")
        ...
        profile_name: str = self.profile_combo.get()
        current_profile: Dict[str, Any] = CONFIG.get('api', {}).get('profiles', {}).get(profile_name, DEFAULT_CONFIG['api']['profiles']['Default'])

        for section_title, section_key in api_sections.items():
            parent: str = self.api_tree.insert("", "end", text=translate(section_title), open=True, iid=section_key)
    ...
    def _rename_profile(self) -> None:
        ...
        if not old_name or old_name == "Default":
            ...
            return
        new_name: Optional[str] = askstring(translate("Rename"), f"{translate('Name:')}:", initialvalue=old_name)
    ...
    def _reset_selected_api(self) -> None:
        ...
        if self.current_api_key_path:
            default_value: Any = _get_nested_value(DEFAULT_CONFIG, ("api", "profiles", "Default") + self.current_api_key_path[3:], "")
            base_url_default: str = DEFAULT_CONFIG['api']['profiles']['Default']['base_url']
    ...
    def load_settings(self, config: Dict[str, Any]) -> None:
        ...
        if hasattr(self, '_update_lang_vars_select_all'):
            self._update_lang_vars_select_all()  # type: ignore[attr-defined]
        if hasattr(self, '_update_currency_vars_select_all'):
            self._update_currency_vars_select_all()  # type: ignore[attr-defined]
        if hasattr(self, '_update_tab_vars_select_all'):
            self._update_tab_vars_select_all()  # type: ignore[attr-defined]
    ...
    def re_translate(self) -> None:
        ...
        # Re-build dynamic lists
        self._build_general_settings(self.general_tab)
        # Reload settings to apply new translations to dynamic lists
        self.load_settings(self.config_manager.get_config())

        # Re-translate API tab
