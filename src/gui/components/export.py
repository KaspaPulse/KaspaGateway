...
class ExportComponent(ttk.Frame):
    def __init__(self, parent, export_callback: Callable[[str], None]):
        ...
        self.csv_button = ttk.Button(self, text=translate("Save as CSV"), command=lambda: self.export_callback('csv'), bootstyle="info")
        self.csv_button.pack(side=LEFT, padx=5)
        
        self.html_button = ttk.Button(self, text=translate("Save as HTML"), command=lambda: self.export_callback('html'), bootstyle="info")
        self.html_button.pack(side=LEFT, padx=5)
        
        self.pdf_button = ttk.Button(self, text=translate("Save as PDF"), command=lambda: self.export_callback('pdf'), bootstyle="info")
