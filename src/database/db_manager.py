...
class DatabaseManager:
    ...
    def __init__(self) -> None:
        ...
        logger.info(f"DB Manager initialized: Data='{self.data_dir}', Backups='{self.backup_dir}'")
    ...
    def get_database_info(self) -> List[Dict[str, Any]]:
        ...
        """
        db_files: List[str] = [f for f in os.listdir(self.data_dir) if f.endswith('.duckdb')]
            ...
            try:
                ...
                if os.path.exists(wal_path):
                    try:
                        ...
                        logger.warning(f"Could not get stats for WAL file {wal_path}: {wal_e}")
    ...
    def backup_database(self, db_name: str) -> Tuple[bool, str]:
        ...
        backup_name: str = f"{os.path.splitext(db_name)[0]}_{datetime.now().strftime('%Y%m%d%H%M%S')}.duckdb"
    ...
    def restore_database(self, backup_file_path: str, target_db_name: str) -> Tuple[bool, str]:
        """
        ...
        try:
            ...
            logger.info(f"Starting file copy restore for '{target_db_name}' from '{backup_filename}'...")
    ...
    def delete_database(self, db_name: str) -> Tuple[bool, str]:
        ...
        try:
            if os.path.exists(db_path):
                ...
                logger.debug(f"File '{db_name}' exists. Size before delete: {size_before:.2f} KB")
                os.remove(db_path)
            ...
            if os.path.exists(db_path):
                ...
                logger.debug(f"File '{db_name}' re-initialized. Size after create: {size_after:.2f} KB")
            ...
            logger.error(f"DELETE FAILED: PermissionError for {db_name}. The file is likely still locked by the application. Error: {e}", exc_info=True)
            return False, f"{translate('Delete failed: PermissionError')}. {translate('Restart the application and try again.')}"
        except Exception as e:
